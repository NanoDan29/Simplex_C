 ...
 * @author Elirpme
 */
public class Transformacion_FuncionesTabla {
    ...
    public String funInvertida[];

    public Transformacion_FuncionesTabla(String ant, int metodo) {
        this.listVariblesDeCambio = new ArrayList<>();
        if (metodo == 0) {
            this.listDiccionario = diccionarioFunciones(ant);
        }else if (metodo == 1) {
            calculoMatrizInvertida_simplexDual(ant);
            this.diccVariablesExis = null;
            this.listVariablesExis = null;
            this.listVariblesDeCambio  = new ArrayList<>();
            this.listDiccionario = diccionarioFuncionesDual(getFunInvertida()[1]);
            //this.listDiccionario = diccionarioFuncionesInvertirDual(ant);
        }
    ...
    public void calculoMatrizInvertida_simplexDual(String ant){
        this.listDiccionario = diccionarioFuncionesInvertirDual(ant);
         this.diccVariablesExis = list_Dic(listVariablesExis);
        //Agregamos la ultima variable existente que es R
        //esto lo hacemos porque al momento de ordenar R no quedaria al final
        ...
        //asi que nos toca agregar a R al diccionario y su valor va ser 
        //la ultima posicion 
        generarTablaFunciones gt = new generarTablaFunciones(this.listVariablesExis, this.getListVariblesDeCambio().size(), 1);
        this.funInvertida = gt.generarMatrizInvertidaSimplexDual(getListVariablesExis().get(0), diccVariablesExis, listDiccionario, listVariblesDeCambio);
        System.out.println("v[0] = "+this.funInvertida[0]);
        System.out.println("v[1] = "+this.funInvertida[1]);
    }
...
//    }
    ...
    public List<Hashtable<String, Double>> diccionarioFuncionesDual(String ent) {
        List<Hashtable<String, Double>> listDiccionario = new ArrayList<>(); //lista de diccionarios
        Hashtable<String, Double> dic = null; //diccionario
        ent = ent.replace(" ", "");
        String funciones[] = ent.split("\n"); // separo las funciones mediante "\n"
    ...
    public List<Hashtable<String, Double>> diccionarioFuncionesInvertirDual(String ent) {
        List<Hashtable<String, Double>> listDiccionario = new ArrayList<>(); //lista de diccionarios
        Hashtable<String, Double> dic = null; //diccionario
        ent = ent.replace(" ", "");
        String funciones[] = ent.split("\n"); // separo las funciones mediante "\n"
        ...
        int cont_A = 1;
        int cont_E = 1;
        boolean bool_cambio_igualacion = false;
        for (String r : funciones) {
            dic = new Hashtable<>(); // inicializo el diccionario
            setDespeje(false);
            for (int i = 0; i < r.length(); i++) {
...
//                        setDiccionario(dic, "S" + cont_S); // al encontrar <= debemos poner las variables de holgura
                        ...
                        cont_S++;
                        i += 2;
                        bool_cambio_igualacion = true;
                        this.listVariblesDeCambio.add("<=");// <= porque multiplicamos por -1 por lo cual cambia la igualacion
                    }
                }
                aux += r.charAt(i) + ""; // acumulador 
            }
            ...
            aux = "";
            listDiccionario.add(dic);// agregamos el diccionario a la lista
            if (bool_cambio_igualacion) { // cuando encontramos >= entonces todos sus valores le multiplicamos por -1
                for (Map.Entry<String, Double> entry : dic.entrySet()) {
                    if (!"S".equals(entry.getKey().substring(0,1))) {
                        dic.put(entry.getKey(), (-1*entry.getValue()));
                    }                    
     ...
     * transforma en 2X1 si se encuentra en la parte derecha despues de haber encontrado
     * un = , <= o >=
...
//    }
    ...
    public String[] getFunInvertida() {
        return funInvertida;
    }

    public void setFunInvertida(String[] funInvertida) {
        this.funInvertida = funInvertida;
    }
